...
winCount = [0,0,0,0,0]
Names = [     'Trevor',       'John',      'Tristan',   'Allen', 'Jonah']
Strategies = ['LionFish', 'ScoreBet'   , 'RiskReward','AllIn', 'Fish']
...
for x in range(gameCount):
    ...
    player1=Hand(Names[0], table, Strategies[0])
    player2=Hand(Names[1], table, Strategies[1])
    player3=Hand(Names[2], table, Strategies[2])
    player5=Hand(Names[3], table, Strategies[3])
    player4=Hand(Names[4], table, Strategies[4])
...
for i in range(len(Names)):
    print(winCount[i],'  ',100*winCount[i]/gameCount,'% :',Names[i],' using strat: ', Strategies[i])
overAllWinner = Names[winCount.index(max(winCount))]
